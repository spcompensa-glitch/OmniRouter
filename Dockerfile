...
RUN --mount=type=cache,id=apt-base,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=apt-lists-base,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
  && apt-get upgrade -y \
...
RUN --mount=type=cache,id=apt-builder,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=apt-lists-builder,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
...
RUN --mount=type=cache,id=apt-web,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=apt-lists-web,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
...
RUN --mount=type=cache,id=apt-cli,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=apt-lists-cli,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
...
RUN --mount=type=cache,id=npm-cli,target=/root/.npm \
